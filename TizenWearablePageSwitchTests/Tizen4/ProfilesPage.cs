...
namespace TizenWearablePageSwitchTests.Tizen4
{
	internal class ProfilesPage : CirclePage, IPageBase
	{
		...
		private readonly CircleListView _listView;
...
""profiles"":
	{
		...
		""name"":
		{
			...
			void ListCellTap(object commandParameter)
			{
				...
				_pageParameters.PageSwitch(editPageParameters);
			}
		}
	...
	public class LabelButtonCell : ViewCell
	{
		...
		protected LabelButtonCell(bool dualLines, bool imageButton)
		{
			...
			//primaryText.MeasureInvalidated += (sender, args) =>
			//{
			...
			//	if (primaryMeasure.Request.Width > stackMeasure.Request.Width - 5)
			//	{
			//		if (primaryText.FontSize <= MinimumTextSize)
	...
	public class ListCellDataTemplateSelector : DataTemplateSelector
	{
		...
		protected override DataTemplate OnSelectTemplate(object item, BindableObject container) => ((LabelButtonCellViewModel)item).DualLines ? _twoLabelButtonCell : _oneLabelButtonCell;
	}
}